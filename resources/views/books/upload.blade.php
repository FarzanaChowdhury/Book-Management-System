<x-layout>
    ...
    <form action="{{ route("books.store")}}" method="POST" class="form-control shadow p-3 mb-5 bg-white rounded" enctype="multipart/form-data">
        ...
        <div class="form-group m-6 p-6">
            ...
            <input type="file" accept="image/*" id="cover_image" class="form-control" name="cover_image" required>

            <label for="name">Book Name:</label>
            <input type="text" id="name" class="form-control" name="name" required>

            <label for="author">Author Name:</label>
            <input type="text" id="author" class="form-control" name="author"   value="{{ Auth::user()->role === 'author' ? Auth::user()->name : '' }}"  
  @if(Auth::user()->role === 'author') readonly @endif
  required
